...
const Restrocard = () => {
  ...
  const onfilter = (element) => {
    if (
      element.name.toLowerCase().includes(searchKey.toLowerCase().trim()) &&
      // || element.address.toLowerCase().includes(searchKey.toLowerCase().trim())
      Number(element.rating) >= Number(rating)
    ) {
      return true;
    } else {
      return false;
  ...
  return (
    <>
      ...
      >
        {restaurantList.filter(onfilter).map((element) => {
